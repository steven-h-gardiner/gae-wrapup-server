...
class XMPPServiceImpl implements XMPPService {
  ...
  public List<Presence> getPresence(Iterable<JID> jabberIds, JID fromJid) {
    ...
    checkArgument(!request.jids().isEmpty(), "Jabber ID list cannot be empty");
      ...
      switch (XmppServiceError.ErrorCode.valueOf(ex.getApplicationError())) {
        case INVALID_JID:
          throw new IllegalArgumentException(
              "One or more jabber ID is invalid: " + jabberIds);
        case NONDEFAULT_MODULE:
          throw new UnsupportedOperationException("XMPP API does not support modules");
        case NO_BODY:
        case UNSPECIFIED_ERROR:
  ...
  public SendResponse sendMessage(Message message) {
    ...
    checkArgument(message.getBody() != null && !message.getBody().isEmpty(),
