 ...
 * {@link #leaseTasks(long, TimeUnit, long) leaseTasks} to acquire a lease on
 * the task and process them explicitly.  Attempting to call
 * {@link #leaseTasks(long, TimeUnit, long) leaseTasks} on a push queue causes
 * a {@link InvalidQueueModeException} to be thrown. When the task processing
...
public interface Queue {
   ...
   * of a task that was previously created, and if no other {@code Exception} would be thrown. Note
   * that if a {@code TaskAlreadyExistsException} is caught, the caller can be guaranteed that for
   * each one of the provided {@code TaskOptions}, either the corresponding task was successfully
   * added, or a task with the given name was successfully added in the past.
   * @throws TransientFailureException Attempting the request after this exception may succeed.
   * @throws UnsupportedTranslationException If chosen character encoding is unsupported.
   * @throws InvalidQueueModeException task method is {@link TaskOptions.Method#PULL} and queue is
   * push queue or vice versa.
   ...
   * options.
   * <p>This method is similar to calling {@link #addAsync(TaskOptions)} with
   * a {@link TaskOptions} object returned by
   * {@link TaskOptions.Builder#withDefaults()}.
   ...
   * @throws UnsupportedTranslationException If chosen character encoding is unsupported.
   */
  Future<List<TaskHandle>> addAsync(
      Transaction txn, Iterable<TaskOptions> taskOptions);
   ...
   *        by a list of TaskHandles. This method supports deleting up to 1000 tasks.
   * @param taskHandles list of handles of tasks to delete.
   * @return {@code List<Boolean>} that represents the result of deleting each task in
   ...
   * @param taskName name of the task to delete.
   * @return A {@code Future} whose result is True if the task was successfully deleted,
   * False if the task was not found or was previously deleted.
   * @throws IllegalArgumentException if the provided name is null, empty or doesn't match the
   *         expected pattern.
   */
  ...
  /**
   * Asynchronously deletes a list of tasks from this {@link Queue}. The tasks are identified
   *        by a list of TaskHandles. This method supports deleting up to 1000 tasks.
   * @param taskHandles list of handles of tasks to delete.
   ...
   *         successfully deleted. False if the task was not found or was previously deleted.
   * @throws IllegalArgumentException if the provided name is null, empty or doesn't match the
   *         expected pattern.
   * @throws QueueNameMismatchException if the task handle refers to a different named queue.
   */
   ...
   * @throws IllegalArgumentException if {@literal lease < 0}, {@literal countLimit <= 0},
   * or either is too large.
   * @throws InternalFailureException
   * @throws IllegalStateException If the queue does not exist. (see queue.xml)
   * @throws TransientFailureException Attempting the request after this exception may succeed.
   */
   ...
   * <p> If {@code options} specifies a tag, only tasks having that tag will be returned.
   * If {@code options} specifies no tag, but does specify {@code groupByTag}, only tasks
   ...
   * or either is too large.
   */
  Future<List<TaskHandle>> leaseTasksAsync(
   ...
   * {@link #leaseTasks(LeaseOptions) leaseTasks}.
   ...
   * @param deadlineInSeconds the maximum duration, in seconds, that the fetch statistics
   *        request can run. A default deadline will be used if {@code null} is supplied.
   ...
   */
  Future<QueueStatistics> fetchStatisticsAsync( Double deadlineInSeconds);
}
