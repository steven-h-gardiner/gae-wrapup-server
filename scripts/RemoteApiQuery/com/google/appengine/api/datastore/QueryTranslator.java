...
final class QueryTranslator {
  ...
  public static DatastoreV3Pb.Query convertToPb(Query query, FetchOptions fetchOptions) {
    ...
    if (fetchOptions.getStartCursor() != null) {
      if (!proto.getMutableCompiledCursor().parseFrom(
          fetchOptions.getStartCursor().toByteString())) {
        throw new IllegalArgumentException("Invalid cursor");
      }
    }

    if (fetchOptions.getEndCursor() != null) {
      if (!proto.getMutableEndCompiledCursor().parseFrom(
          fetchOptions.getEndCursor().toByteString())) {
        throw new IllegalArgumentException("Invalid cursor");
      }
    }
  ...
  /**
   * Converts the filter from a geo-spatial query into proto-buf form.
   * Should only be called when the filter indeed has a geo-spatial
   * term; but the filter as a whole has not yet been entirely
  ...
  private static void copyGeoFilterToPb(Query.Filter filter, DatastoreV3Pb.Query proto) {
    if (filter instanceof Query.CompositeFilter) {
      ...
      checkArgument(conjunction.getOperator() == Query.CompositeFilterOperator.AND,
      ...
      checkArgument(predicate.getOperator() == Query.FilterOperator.EQUAL,
