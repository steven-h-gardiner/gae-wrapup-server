...
public interface IImagesServiceFactory {
   ...
   * @throws BlobstoreFailureException If there is an error obtaining the Google
   * Storage access token for the {@code filename}.
   */
   ...
   * {@code cropOffsetY} are outside the range 0.0 to 1.0.
   */
   Transform makeResize(int width, int height, double cropOffsetX,
                                     double cropOffsetY);
   ...
   */
   Transform makeCrop(float leftX, float topY, float rightX,
                                   float bottomY);
   ...
   * range 0.0 to 1.0 or if {@code leftX >= rightX} or {@code topY >= bottomY}.
   */
   Transform makeCrop(double leftX, double topY,
   ...
   */
   Composite makeComposite(Image image, int xOffset, int yOffset,
                                        float opacity,
