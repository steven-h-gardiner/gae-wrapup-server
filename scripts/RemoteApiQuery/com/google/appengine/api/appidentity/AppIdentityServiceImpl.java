...
class AppIdentityServiceImpl implements AppIdentityService {
  ...
  private static LoadingCache<String, CacheItem> cache = CacheBuilder.newBuilder()
      .maximumSize(MAX_INSTANCE_CACHE_ENTRIES)
      .build(new CacheLoader<String, CacheItem>() {
        @Override public CacheItem load(String key) {
  ...
  private static class CacheItem {
    ...
    public GetAccessTokenResult get() {
  ...
  public String getDefaultGcsBucketName() {
    ...
    try {
      responseBuilder.mergeFrom(
          ApiProxy.makeSyncCall(
              PACKAGE_NAME, GET_DEFAULT_GCS_BUCKET_NAME,
              requestBuilder.build().toByteArray()));
    } catch (ApiProxy.ApplicationException e) {
      handleApplicationError(e);
    } catch (InvalidProtocolBufferException e) {
