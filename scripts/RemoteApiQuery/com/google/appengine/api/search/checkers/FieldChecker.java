...
public final class FieldChecker {
  ...
  public static String checkPrefix(String prefix) {
    if (prefix != null) {
      Preconditions.checkArgument(bytesInString(prefix) <= SearchApiLimits.MAXIMUM_PREFIX_LENGTH,
          "Field prefix longer than maximum length %d", SearchApiLimits.MAXIMUM_PREFIX_LENGTH);
    }
    return prefix;
  }
  ...
  public static List<Double> checkVector(List<Double> vector) {
    Preconditions.checkArgument(vector.size() <= SearchApiLimits.VECTOR_FIELD_MAX_SIZE,
        "Vector size greater than %d", SearchApiLimits.VECTOR_FIELD_MAX_SIZE);
    for (Double d : vector) {
