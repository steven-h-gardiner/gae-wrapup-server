...
<xsl:stylesheet version="1.0"
  ...
  <xsl:param name="rezurl" />
  <xsl:param name="taskid" />
  <xsl:param name="rezdoc">
    <xsl:choose>
      <xsl:when test="$rezurl != ''">
	<xsl:copy-of select="document($rezurl)"/>
	<xsl:message>
	  REZ
	  <xsl:value-of select="document($rezurl)//resource/@taskid"/>
	</xsl:message>
      </xsl:when>
    </xsl:choose>
  </xsl:param>
  ...
  <xsl:template match="@*" mode="resolve">
	...
	<xsl:when test="$rezurl != ''">
	  <xsl:variable name="resource">
	    <xsl:value-of select="document($rezurl)//resource[@relpath=$raw0][@taskid=$taskid]/@locpath"/>
	  </xsl:variable>	 
	  <xsl:message>
	    ABS <xsl:value-of select="$taskid"/>/<xsl:value-of select="$resource"/>
	  </xsl:message>
...
<![CDATA[
  ...
  <xsl:template match="html:link[contains(@href,'CACHE/css/a0ba')]" />

  <xsl:template match="html:a/@draggable" />

  <xsl:template match="html:div[@class='_bento']//html:div[@class='controls']" />

  <xsl:template match="html:div[@id='mainmenumid']/html:ul[@id='udm']" />

  <xsl:template match="html:i/html:img/@class[.='offscreen1']" />
  <xsl:template match="html:i[html:img/@class='offscreen']">
    <xsl:copy>
      <xsl:attribute name="style">
	<xsl:text>background-image: url(http://s3-media4.fl.yelpcdn.com/assets/srv0/yelp_styleguide/c2252a4cd43e/assets/img/stars/stars_map.png); </xsl:text>
      </xsl:attribute>
      <xsl:apply-templates select="@*"/>
      <xsl:apply-templates select="node()"/>
    </xsl:copy>	
  ...
  <xsl:template match="html:body/html:div[@class='page']">
    <xsl:copy>
      <xsl:attribute name="style">
